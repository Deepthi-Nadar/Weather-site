...
function setNowIcon(code, isDay, el){
  ...
  if (name === 'clear-day') {
    src = './icons/clear-day.svg/clear.svg';
  } else if (name === 'clear-night') {
    src = './icons/clear-night.svg/clear-night.svg';
  }
...
function displayHistoricalDay(data, wrap){
  ...
  const timeDate = new Date(timeStr).toLocaleString(undefined, {weekday:'long', month:'long', day:'numeric', year:'numeric', hour:'numeric', minute:'2-digit'});
...
async function search(){
  ...
  try {
    ...
    await updateAll({ name: p.name, lat: p.latitude, lon: p.longitude });
  } catch(e){
...
$('#histSearch').addEventListener('click', async () => {
  ...
  const timeInput = $('#histTime').value;
  showLoader();
  try {
    const histData = await fetchHistorical(currentLat, currentLon, dateInput, timeInput);
...
(async function init() {
  ...
  try {
    ...
    if (!place) {
      const mumbai = await geocodeCity('Mumbai');
      place = { name: mumbai.name, lat: mumbai.latitude, lon: mumbai.longitude };
    }
    ...
    const mumbai = await geocodeCity('Mumbai');
    await updateAll({ name: mumbai.name, lat: mumbai.latitude, lon: mumbai.longitude });
  } finally {
    hideLoader();
  }
  populateFavorites();
